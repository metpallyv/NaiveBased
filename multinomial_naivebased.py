...
def add_word_to_appropriate_class_dict(words):
    ...
    if doc_class_val == 1:
        if class_1.has_key(words[1]):
            ...
            class_1[words[1]] =  words[2]
    elif doc_class_val == 2:
        if class_2.has_key(words[1]):
            class_2[words[1]] = int(class_2.get(words[1])) + int(words[2])
        else:
            class_2[words[1]] = words[2]
    elif doc_class_val == 3:
        if class_3.has_key(words[1]):
            class_3[words[1]] = int(class_3.get(words[1])) + int(words[2])
        else:
            class_3[words[1]] = words[2]
    elif doc_class_val == 4:
        if class_4.has_key(words[1]):
            class_4[words[1]] = int(class_4.get(words[1])) + int(words[2])
        else:
            class_4[words[1]] = words[2]
    elif doc_class_val == 5:
        if class_5.has_key(words[1]):
            class_5[words[1]] = int(class_5.get(words[1])) + int(words[2])
        else:
            class_5[words[1]] = words[2]
    elif doc_class_val == 6:
        if class_6.has_key(words[1]):
            class_6[words[1]] = int(class_6.get(words[1])) + int(words[2])
        else:
            class_6[words[1]] = words[2]
    elif doc_class_val == 7:
        if class_7.has_key(words[1]):
            class_7[words[1]] = int(class_7.get(words[1])) + int(words[2])
        else:
            class_7[words[1]] = words[2]
    elif doc_class_val == 8:
        if class_8.has_key(words[1]):
            class_8[words[1]] = int(class_8.get(words[1])) + int(words[2])
        else:
            class_8[words[1]] = words[2]
    elif doc_class_val == 9:
        if class_9.has_key(words[1]):
            class_9[words[1]] = int(class_9.get(words[1])) + int(words[2])
        else:
            class_9[words[1]] = words[2]
    elif doc_class_val == 10:
        if class_10.has_key(words[1]):
            class_10[words[1]] = int(class_10.get(words[1])) + int(words[2])
        else:
            class_10[words[1]] = words[2]
    elif doc_class_val == 11:
        if class_11.has_key(words[1]):
            class_11[words[1]] = int(class_11.get(words[1])) + int(words[2])
        else:
            class_11[words[1]] = words[2]
    elif doc_class_val == 12:
        if class_12.has_key(words[1]):
            class_12[words[1]] = int(class_12.get(words[1])) + int(words[2])
        else:
            class_12[words[1]] = words[2]
    elif doc_class_val == 13:
        if class_13.has_key(words[1]):
            class_13[words[1]] = int(class_13.get(words[1])) + int(words[2])
        else:
            class_13[words[1]] = words[2]
    elif doc_class_val == 14:
        if class_14.has_key(words[1]):
            class_14[words[1]] = int(class_14.get(words[1])) + int(words[2])
        else:
            class_14[words[1]] = words[2]
    elif doc_class_val == 15:
        if class_15.has_key(words[1]):
            class_15[words[1]] = int(class_15.get(words[1])) + int(words[2])
        else:
            class_15[words[1]] = words[2]
    elif doc_class_val == 16:
        if class_16.has_key(words[1]):
            class_16[words[1]] = int(class_16.get(words[1])) + int(words[2])
        else:
            class_16[words[1]] = words[2]
    elif doc_class_val == 17:
        if class_17.has_key(words[1]):
            class_17[words[1]] = int(class_17.get(words[1])) + int(words[2])
        else:
            class_17[words[1]] = words[2]
    elif doc_class_val == 18:
        if class_18.has_key(words[1]):
            class_18[words[1]] = int(class_18.get(words[1])) + int(words[2])
        else:
            class_18[words[1]] = words[2]
    elif doc_class_val == 19:
        if class_19.has_key(words[1]):
            class_19[words[1]] = int(class_19.get(words[1])) + int(words[2])
        else:
            class_19[words[1]] = words[2]
    elif doc_class_val == 20:
        if class_20.has_key(words[1]):
            class_20[words[1]] = int(class_20.get(words[1])) + int(words[2])
        else:
...
def predict_class(vocab_words,v):
    ...
    len_class = []
    for c in class_list:
        lis = [int(i) for i in c.values()]
        len_class.append(sum(lis))
        ...
        for it,val in enumerate(cond_prob_class_list):
            ...
            for term in vocab_words:
                if ele_in_doc.has_key(term):
                    count = int(ele_in_doc[term])
                else:
                    count = 0
                score += count * int(val[term])
            if score > max_score:
                max_score = score
                final_class = it+1
...
'''def predict_class(vocab_words,v):
    ...
    for v in l:
        vocab_words = []
        for i in range(v):
            vocab_words.append(Freq[i])
    max_score = 0
    pred_class = 0
    for i in class_list:
        score,cl = predict_class_for_top_words(vocab_words,v)
        if(score > max_score):
            max_score = score
            pred_class = cl
        return pred_class'''
...
def cal_conditional_term_in_vocab(vocab):
    for it,val in enumerate(class_list):
        ...
        for term in vocab:
            if val.has_key(term):
                tf = int(val[term])
                cond_prob_class[term]= math.log(tf+1)- math.log(len_c+len(vocab))
            else:
                tf = 0
...
if __name__ == "__main__":
    if len(sys.argv) == 5:
        ...
        for v in l:
            ...
            build_multinomial_distr(train_file,vocab_words)
            cond_prob_class_1 = {}
            cond_prob_class_2 = {}
            cond_prob_class_3 = {}
            cond_prob_class_4 = {}
